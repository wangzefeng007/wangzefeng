<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('post_forumselect');?><?php include template('common/header'); ?><script src="<?php echo $_G['setting']['jspath'];?>common.js?<?php echo VERHASH;?>" type="text/javascript"></script>

<!-- header start -->
<div class="n5-header">
    <a class="n5-cebianlan" id="open-sb"><i class="n5-ico"></i></a>
<a class="n5-logo" href="forum.php?mod=guide&amp;view=hot"><img src="<?php echo $_G['style']['logo地址'];?>" alt="<?php echo $_G['setting']['sitename'];?>" width="100%"></a>
<a class="n5-fatie" href="forum.php?mod=misc&amp;action=nav" rel="nofollow"><i class="n5-ico"></i></a>
</div><?php include template('common/n5-cbl'); ?><!-- header end -->
...
<button id="postbtn" class="n5-qh n5-fq" onclick="hideWindow('nav');showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=' + selectfid)" disabled="disabled"><span>发新帖</span></button>
<?php } else { ?>
<button id="postbtn" class="n5-qh n5-bk" onclick="hideWindow('nav');window.location.href='forum.php?mod=post&action=newthread&fid=' + selectfid + '&special=<?php echo $special;?>'" disabled="disabled"><span><?php echo $actiontitle;?></span></button>
<?php } } ?>
...
<a href="<?php if($_G['uid']) { ?>home.php?mod=space&uid=<?php echo $_G['uid'];?>&do=profile&mycenter=1<?php } else { ?>member.php?mod=logging&action=login<?php } ?>" class="bottom_member"></a>
