<?php exit;?>
<!--{if $do == 'feed'}-->
<!--{template common/header_toptb}-->
	<style id="diy_style" type="text/css"></style>
	<div class="wp">
		<!--[diy=diy1]--><div id="diy1" class="area"></div><!--[/diy]-->
	</div>
	<div id="tct" class="ct2_a wp cl" style="padding:0;width:auto !important">
			<div class="appl">
<!--{if $_G['uid']}-->
...
<!--{/if}-->
			<div><div class="avtss" style="width: 100px;Float:left;"><a href="home.php?mod=space&uid=$_G[uid]">
			<div class="avtbg" style="width: 100px;height: 100px;margin: 0 !important;border-radius:5px"><!--{avatar($_G[uid],big)}--></div></a></div>
            <div class="name" style="margin-left:8px;float:left;width: 90px;margin-top: 5px;font-weight: bold;height:95px">
                  <a href="home.php?mod=space&uid=$_G[uid]" style="float:left;width: 100%">{$_G[member][username]}</a>
				  <th class="alt"><span class="notice" style="float:left;font-weight: normal;background:none;padding-left:0;width: 100%">{lang credits}: $space[credits]</span></th>
				  $diynav
			 </div>
             </div>
	...
	<!--{else}-->
		...
		<div class="mtn ctrl flw_hd" style="width: 300px;margin-top: 10px !important;">
			<!--{if !$space[self]}-->
...
<!--{/if}-->
				...
				<!--{subtemplate common/userabout}-->
					<hr class="da" style="margin-top:10px">
						<div class="flw_user_list mbm">
							<h3 class="xi2 xs2" style="margin-top:10px">{lang follow_recommend}</h3>
							<ul class="mls cl">
							<!--{loop $recommend $ruid $rusername}-->
								<li>
									...
									<!--{if helper_access::check_module('follow')}-->
									<span><a id="a_followmod_{$ruid}" href="home.php?mod=spacecp&ac=follow&op=add&hash={FORMHASH}&fuid=$ruid&from=block" onclick="ajaxget(this.href);doane(event);" style="text-decoration: none !important;background-color: #BDD3E0;color: white !important;padding: 1px 5px;margin-top: 4px" class="tspan">{lang follow_add}</a></span>
									<!--{/if}-->
								</li>
							<!--{/loop}-->
							</ul>
		...
		<div class="mn" id="tmn" style="width:691px !Important;border:0;padding-right: 0">
...
<!--{/if}-->
		...
		<!--{if in_array($do, array('feed', 'view'))}-->
			<!--{if helper_access::check_module('follow') && ( $do == 'feed' || ( $do == 'view' && $viewself))}-->
				<div id="flw_header" class="cl">
					...
					<!--{eval $dmfid = $_G['setting']['followforumid'] && !empty($defaultforum) ? $_G['setting']['followforumid'] : 0;}-->
					  ...
					  {if $_G[uid] && isset($_G[cookie][extstyle]) && strpos($_G[cookie][extstyle], TPLDIR) !== false}<img id="home_hear_bg" src="$_G[cookie][extstyle]/fow_top.png" style="float:left;z-index:10;position: relative;"/>{else}<img id="home_hear_bg" src="./template/enet_no1/images/fow_top.png" style="float:left;z-index:10;position: relative;"/>{/if}
					<div class="fow_right" style="float:right;position: relative;margin-top:4px;height:30px;z-index:10"><span style="float:right;line-height: 30px;">{lang spacecp_doing_message2}</span><span id="checklens" class="xg1" style="float:right;height:30px;line-height: 22px;font-size:26px">2014</span><span style="float:right;line-height: 30px;">{lang spacecp_doing_message1}</span></div>			
					<form method="post" autocomplete="off" id="fastpostform" action="home.php?mod=spacecp&ac=follow&op=newthread&topicsubmit=yes&infloat=yes&handlekey=fastnewpost&inajax=1" onsubmit="return fastpostvalidate(this);" style="float: left;margin-top:3px">
						...
						<div id="flw_post_extra" class="mtn cl">
						  <div{if $_G[setting][fastsmilies]} class="hasfsl"{/if} id="fastposteditor">
							<div class="tedt">
									<div class="area">
										<textarea rows="5" cols="80" name="message" id="fastpostmessage" onKeyDown="seditor_ctlent(event, '$(\'fastpostsubmit\').click()');" tabindex="12" onkeyup="strLenCalc(this, 'checklens', 2014);" class="pt xs2"></textarea>
									<div class="talkss"></div>
									</div>
							...
							<!--{/if}-->
							<div class="cl" style="float:right;width:442px">
								<div class="y">
								<p class="ptm pnpost">
								...
								<div class="z ptm" id="forumlistdev" style="display: none;padding-top:10px !important">
									<select name="defaultforum" style="float:right" id="fid" class="ps z" onchange="modifyformurl(this.value);">
										<option value="0">{lang follow_other_forum}</option>
									</select>
									<select name="forumlist" id="forumlist" style="float:right" class="ps z" onchange="addforumlist(this);" style="display: none;">
					...
					function succeedhandle_fastnewpost(url, msg, values) {
						if(nofollowfeed) {
							...
							//var msg = '您的应用已发布，<a href="'+url+'" class="xi2">点击这里查看应用</a>'
							//showDialog(msg, 'notice', null, null, 0, null, null, null, null, 3);
						}
			...
			<!--{/if}-->
						<div class="fow_ad" style="background: url(template/enet_no1/images/fow_ad.png) no-repeat;width:651px;height:100px;margin-left:20px;margin-bottom:10px"></div>
			<!--{if in_array($do, array('feed', 'view'))}-->
			<div style="width:691px">
				...
				<!--{if !empty($list['feed'])}-->
					<div class="flw_feed" style="width:691px">
						<ul id="followlist">
