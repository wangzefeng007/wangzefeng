...
<!--{elseif $_GET['op'] == 'docomment' || $_GET['op'] == 'getcomment'}-->
	...
	<div id="{$_GET[key]}_form_{$doid}_{$id}">
		<form id="{$_GET[key]}_docommform_{$doid}_{$id}" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=doing&op=comment&doid=$doid&id=$id" {if $_G[inajax]}onsubmit="ajaxpost(this.id, 'return_$_GET[handlekey]');"{/if} class="pns" style="margin: 5px 0 0;">
			<div style="height:110px"><div class="p_div_text bm"><textarea name="message" id="{$_GET[key]}_form_{$doid}_{$id}_t" cols="40" class="p_textarea" oninput="resizeTx(this);" onpropertychange="resizeTx(this);" onkeyup="strLenCalc(this, '{$_GET[key]}_form_{$doid}_{$id}_limit')" onkeydown="ctrlEnter(event, '{$_GET[key]}_replybtn_{$doid}_{$id}');"></textarea></div>
			...
			<div class="y" style="width:300px;float:right">
			<button type="submit" name="do_button" id="{$_GET[key]}_replybtn_{$doid}_{$id}" class="pn pnc p_but" value="true" style="float:right;margin-right:0"><em>{lang reply}</em></button>
			<!--{if $_G[inajax]}--><input type="hidden" name="handlekey" value="$_GET[handlekey]" /><!--{/if}-->
			<a name="btncancel" class="p_cancel pn" href="javascript:;" onclick="docomment_form_close($doid, $id, '$_GET[key]');" style="margin-right:10px;margin-left:8px;float:right;height: 25px !Important;display: block;"><strong style="line-height:27px;color:#777 !Important">{lang cancel}</strong></a>
			<span id="{$_GET[key]}_form_{$doid}_{$id}_t_limit" style="display: none;float:right">{lang spacecp_doing_message1} <span id="{$_GET[key]}_form_{$doid}_{$id}_limit">200</span> {lang spacecp_doing_message2}</span>
			<input type="hidden" name="formhash" value="{FORMHASH}" />
			</div>
			</div>
		</form>
