...
<div id="ct" class="ct7_a wp cl" style="margin-top:10px">
<div class="apps">
		<div class="tbn" style="margin:0">
			<h2 class="mt">{lang forum_manager}</h2>
			<ul>
				...
				<!--{if $modforums['fids']}-->
					<!--{if $_G['group']['allowmodpost'] || $_G['group']['allowmoduser']}-->
						<li{if $_GET[action] == 'moderate'} class="a cl"{else} class="cl"{/if}><span class="y mtn">$modnum</span><a href="{$cpscript}?mod=modcp&action=moderate&op={if $_G['group']['allowmodpost']}threads{$forcefid}{else}members{/if}">{lang forum_moderate}</a></li>
					<!--{/if}-->
				<!--{/if}-->
				...
				<!--{if $_G['group']['allowedituser'] || $_G['group']['allowbanuser'] || $_G['group']['allowbanvisituser'] || $_G['group']['allowbanip']}-->
					...
					<!--{if $modforums['fids']}--><li{if $_GET[action] == 'forumaccess'} class="a"{/if}><a href="{$cpscript}?mod=modcp&action=forumaccess{$forcefid}">{lang mod_option_member_access}</a></li><!--{/if}-->
				...
				<!--{if $_G['group']['allowpostannounce'] || $_G['group']['allowviewlog']}-->
					<!--{if $_G['group']['allowpostannounce']}--><li{if $_GET[action] == 'announcement'} class="a"{/if}><a href="{$cpscript}?mod=modcp&action=announcement$forcefid">{lang announcements}</a></li><!--{/if}-->
