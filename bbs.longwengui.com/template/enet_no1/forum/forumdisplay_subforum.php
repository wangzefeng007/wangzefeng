	...
	<div id="subforum_{$_G[forum][fid]}" class="bm_c" style="$collapse[subforum]">
		<table cellspacing="0" cellpadding="0" class="fl_tb">
				...
				<td class="fl_g" width="$_G[forum][forumcolwidth]">
	               ...
	               <!--{/if}-->
					...
					<dl{if !empty($sub[extra][iconwidth]) && !empty($sub[icon])} style="margin-left: {$sub[extra][iconwidth]}px;"{/if}>
						<dt><a href="$forumurl" {if !empty($sub[redirect])}target="_blank"{/if} style="{if !empty($sub[extra][namecolor])}color: {$sub[extra][namecolor]};{/if};font-size: 17px;font-weight: 100" class="xi2">$sub[name]</a><!--{if $sub[todayposts] && !$sub['redirect']}--><em class="xw0 xi1" title="{lang forum_todayposts}"> ($sub[todayposts])</em><!--{/if}--></dt>
						<!--{if empty($sub[redirect])}--><dd class="xg1"><em><span>{lang forum_threads}:</span><!--{echo dnumber($sub[threads])}--></em><em><span style="margin-left:4px">{lang forum_posts}: </span><!--{echo dnumber($sub[posts])}--></em></dd><!--{/if}-->
						<dd class="xg1">
						<!--{if $sub['permission'] == 1}-->
			...
			<!--{else}-->
				...
				<td>
					<h2><a href="$forumurl" {if !empty($sub[redirect])}target="_blank"{/if} style="{if !empty($sub[extra][namecolor])}color: {$sub[extra][namecolor]};{/if};font-size: 17px;font-weight: 100" class="xi2">$sub[name]</a><!--{if $sub[todayposts] && !$sub['redirect']}--><em class="xw0 xi1" title="{lang forum_todayposts}"> ($sub[todayposts])</em><!--{/if}--></h2>
