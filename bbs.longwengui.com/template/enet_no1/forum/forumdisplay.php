...
<div class="boardnav">
    <div style="background:#fff;border-radius:5px;margin-bottom: 10px">
		<div id="borderbox" class="xs2 bm_h bm" style="margin-top:10px;height:auto;background:#fff;padding:10px;padding-bottom:10px;border-bottom:2px dashed #0ad;border-width:2px;border-color:#0ad;margin-bottom:0;border-radius:5px 5px 0 0 !Important">
					...
					<!--{if !$subforumonly}--><div class="xs1 xw0 i"><div style="float:left">{lang index_today}: <strong class="xi1">$_G[forum][todayposts]</strong><!--{if $_G[forum][todayposts]}--><!--{if $_G[forum][todayposts] < $_G[forum][yesterdayposts]}--><b class="ico_fall">&nbsp;</b><!--{else}--><b class="ico_increase">&nbsp;</b><!--{/if}--><!--{/if}--><span class="pipe"></span>{lang index_threads}: <strong class="xi1">$_G[forum][threads]</strong><!--{if $_G[forum][rank]}--><span class="pipe"></span>{lang rank}: <strong class="xi1" title="{lang previous_rank}:$_G[forum][oldrank]">$_G[forum][rank]</strong><!--{if $_G[forum][rank] <= $_G[forum][oldrank]}--><b class="ico_increase">&nbsp;</b><!--{else}--><b class="ico_fall">&nbsp;</b><!--{/if}--><!--{/if}--></div><!--{if $moderatedby}--><div style="float:right;font-size: 14px">{lang forum_modedby}: <span class="xg1">$moderatedby</span></div><!--{/if}--></div><!--{/if}-->
		...
		<div class="wp">
			<div id="borderbox" class="bm bml pbn" style="height:auto;margin-bottom: 0;padding-right:0;border-top:none !Important;border-width:2px;border-color:#0ad;border-radius:0 0 5px 5px;padding:0">
					...
					<div class="bm_c bm_h cl pbn" id="forum_rules_{$_G[fid]}" style="height:auto;margin-bottom:0;padding:0 10px !Important;$collapse['forum_rules'];">
						...
						<!--{if $_G['page'] == 1 && $_G['forum']['rules']}-->
							<div id="forum_rules_{$_G[fid]}">
								<div class="ptn xg2" style="white-space: normal;padding-top:5px !Important">$_G['forum'][rules]</div>
							</div>
						<!--{/if}-->
					</div>
				 <!--{/if}-->
				...
				<div id="pgt" class="bm bw0 pgs cl" style="padding:10px 12px;margin-bottom: 0px;padding-bottom: 6px;padding-top: 15px">
					...
					<!--{if !$_GET['archiveid']}--><a href="javascript:;" id="newspecial" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} title="{lang send_posts}"><img src="template/enet_no1/images/pn_post.png" alt="{lang send_posts}" /></a><!--{/if}-->
					<!--{hook/forumdisplay_postbutton_top}-->
				</div>
			</div>
			</div>
	</div>
	<div id="ct" class="wp cl{if $_G['forum']['allowside']} ct4{/if}" {if $leftside} style="margin-left:{$_G['leftsidewidth_mwidth']}px"{/if}>
		<!--{if $leftside}-->
		...
		<div class="mn">
			...
			<!--{if !$subforumonly}-->
				...
				<div class="bm bmw fl{if $_G['forum']['forumcolumns']} flg{/if}" style="border-top:1px solid #dfdfdf">
					<div class="bm_h cl">
			...
			<!--{if !$subforumonly}-->
				...
				<!--{if !($_G['forum']['simple'] & 1) && $_G[setting][whosonlinestatus]}-->
					<div class="bm">
						<!--{if $detailstatus}-->
						<div class="bm_h cl">
							<span class="o y" style="width:17px"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]&page=$page&showoldetails=no#online"><img src="{IMGDIR}/collapsed_no_1.gif" alt="" /></a></span>
							<h2 style="font-size:12px">{lang forum_activeusers} ($onlinenum)</h2>
						</div>
						<div class="bm_c">
							<ul class="ml mls cl">
						...
						<!--{else}-->
						<div class="bm_h cl">
							<span class="o y" style="width:17px"><a href="forum.php?mod=forumdisplay&fid=$_G[fid]&page=$page&showoldetails=yes#online" class="nobdr"><img src="{IMGDIR}/collapsed_yes_1.gif" alt="" /></a></span>
							<h2 style="font-size:12px">{lang forum_activeusers} ($onlinenum)</h2>
						</div>
						<!--{/if}-->
					</div>
