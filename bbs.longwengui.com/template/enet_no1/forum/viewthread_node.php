...
<div class="bm <!--{if !IS_ROBOT && $post['first']}-->first_post<!--{/if}-->" style="margin-bottom:10px;background:transparent">
<table class="plhin" summary="pid$post[pid]" cellspacing="0" cellpadding="0">
<tr>
	<!--{if 0}-->
	<!--{/if}-->	
	<td class="plc" style="padding: 0 20px<!--{if $close_leftinfo}-->;width:100%<!--{/if}-->">
		<div class="avt_left pls favatar" style="margin-top:15px;float:left;padding-bottom: 10px;width:90px;<!--{if !IS_ROBOT && $post['first']}-->display:none<!--{/if}-->">
		<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
			<div class="blk bui" id="userinfo$post[pid]" style="display: none;margin-top:-16px !important;margin-left: -21px !important;">
				<div class="z">
					<div id="userinfo$post[pid]_ma" class="avts"></div>
					<!--{hook/viewthread_profileside $postcount}-->
				</div>
				<div class="i y">
					<div>
						<a href="home.php?mod=space&uid=$post[authorid]" target="_blank" class="xi2" style="font-size: 15px">$post[author]</a>
		...
		<!--{/if}-->
		<div style="width: 90px;height: 90px;margin-bottom:5px"><a href="home.php?mod=space&uid=$post[authorid]" target="_blank" onmouseover="showauthor(this, 'userinfo$post[pid]')" class="xi2 avts z" style="margin-right:15px">{echo avatar($post['authorid'], 'middle')}</a></div>
		...
		<div>
		<div class="pi" style="border:0px !important;padding-bottom:3px;overflow: visible;padding-top:15px;margin-left:105px;<!--{if !IS_ROBOT && $post['first']}-->margin-left:0<!--{/if}-->">
			<!--{if !IS_ROBOT}-->
				<strong>
			...
			<div class="pti">
				...
				<div class="authi">
				<a href="home.php?mod=space&uid=$post[authorid]" target="_blank" class="xi2" style="font-size:15px;padding-right:5px;<!--{if !IS_ROBOT && $post['first']}-->display:none<!--{/if}-->">$post[author]</a>$authorverifys
				...
				<!--{if $post['authorid'] && !$post['anonymous']}-->
					...
					<!--{/if}-->
					<em id="authorposton$post[pid]" <!--{if !IS_ROBOT && $post['first']}-->style="display:none"<!--{/if}-->>{lang poston} $post[dateline]</em>
					<!--{if $post['status'] & 8}-->
						<span><!--{if $_G['setting']['mobile']['mobilecomefrom']}-->{$_G['setting']['mobile']['mobilecomefrom']}<!--{else}-->{lang from_mobile}<!--{/if}--></span>
					<!--{/if}-->
					<!--{if $post['invisible'] == 0}-->
		...
		<div class="pct" style="margin-left:105px;<!--{if !IS_ROBOT && $post['first']}-->margin-left:0<!--{/if}-->">
...
<!--{/if}-->
		...
		<div id="comment_$post[pid]" style="position:relative"><!--{if $_G['setting']['commentnumber'] && !empty($comments[$post[pid]])}--><span class="icon_preview" style="top: -6px;left: 27px;z-index:2"></span><div class="cmtl xld" style="padding-left: 22px;margin-bottom: 10px;border: 1px solid #C2D5E3;border-radius: 5px;padding-top: 5px;padding-bottom: 5px;position:relative">
			...
			<!--{/loop}-->
		</div><!--{/if}--><div id="cmtform_$post[pid]"></div></div>
