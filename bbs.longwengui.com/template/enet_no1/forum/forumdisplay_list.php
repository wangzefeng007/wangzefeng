		...
		<table cellspacing="0" cellpadding="0">
			<tr>
				...
				<!--{if empty($_G['forum']['picstyle'])}-->
					...
					<!--{/if}-->
				<td class="num" style="width:120px"><span style="float:right;line-height:15px" id="atarget" {if $_G['cookie']['atarget'] > 0}onclick="setatarget(-1)" class="y atarget_1"{else}onclick="setatarget(1)" class="y"{/if} title="{lang new_window_thread}">{lang new_window}</span></td>
				<!--{else}-->
		...
		<form method="post" autocomplete="off" name="moderate" id="moderate" action="forum.php?mod=topicadmin&action=moderate&fid=$_G[fid]&infloat=yes&nopost=yes">
			...
			<table summary="forum_$_G[fid]" cellspacing="0" cellpadding="0" id="threadlisttableid">
				<!--{if (!$simplestyle || !$_G['forum']['allowside'] && $page == 1) && !empty($announcement)}-->
					<tbody>
						<tr class="anno{if !empty($_G['forum']['picstyle']) && empty($_G['cookie']['forumdefstyle'])} bw0_all{/if}">
							<td class="avt">
								<a><img src="{IMGDIR}/annpm.png" alt="{lang announcement}" /></a>
							</td>
							...
							<th class="lock">
								...
								<p class="mtn xg1">{lang author}:<a href="home.php?mod=space&uid=$announcement[authorid]">$announcement[author]</a>&nbsp;&nbsp;{lang dateline}:$announcement[starttime]</p>
							</th>
							<td class="flt">
							</td>
							<td class="flt">
				...
				<!--{if !$separatepos || !$_G['setting']['forumseparator']}-->
					<tbody id="separatorline" class="emptb"><tr><td class="avt"></td><!--{if !$_GET['archiveid'] && $_G['forum']['ismoderator']}--><td class="o"></td><!--{/if}--><th class="common" style="padding-bottom:0 !Important"></th><td class="flt"></td><td class="flt"></td><td class="by"></td></tr></tbody>
				<!--{/if}-->
				<!--{if $_G['forum_threadcount']}-->
						...
						<!--{loop $_G['forum_threadlist'] $key $thread}-->							
							...
							<tbody id="$thread[id]"{if $_G['hiddenexists'] && $thread['hidden']} style='display:none'{/if}>
								<tr>
									...
									<td class="by">
									<div class="xld" style="float: right"><span class="m hm"><strong class="xi2">$thread[allreplies]</strong><span>{lang reply}</span></span><span class="m hm"><strong class="xi2"><!--{if $thread['isgroup'] != 1}-->$thread[views]<!--{else}-->{$groupnames[$thread[tid]][views]}<!--{/if}--></strong><span>{lang focus_show}</span></span></div>
									</td>
								</tr>
							</tbody>
						<!--{/loop}-->
					...
					<!--{else}-->
						...
						<ul id="waterfall" class="ml waterfall cl" style="margin: 10px auto;">
							<!--{loop $_G['forum_threadlist'] $key $thread}-->
							...
							<li style="width:{$waterfallwidth}px">
								...
								<div class="auth cl">
									...
									<!--{if $thread['authorid'] && $thread['author']}-->
									<!--{if $thread['authorid'] && $thread['author']}-->
									<a href="home.php?mod=space&uid=$thread[authorid]" class="picavt"><!--{avatar($thread[authorid],small)}--></a>
									<!--{else}--><a class="picavt"><img src="$_G['style']['styleimgdir']/hidden.gif" title="$_G[setting][anonymoustext]" alt="$_G[setting][anonymoustext]" width="30" height="30" /></a><!--{/if}--><a href="home.php?mod=space&uid=$thread[authorid]" class="xg1" style="float:left;margin-top:7px;margin-left: 5px;width:80%">$thread[author]</a><span class="date_pic" style="width:80%;float:left;margin-left:5px"><span{if $thread['istoday']} class="xi1"{/if}>$thread[dateline]</span></span><!--{if !empty($verify[$thread['authorid']])}--> <a style="float:left;margin-top: 13px;margin-left: 5px">$verify[$thread[authorid]]</a><!--{/if}-->
									<!--{else}-->
