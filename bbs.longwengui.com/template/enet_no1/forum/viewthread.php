...
<div id="postlist" class="pl bf bm" style="margin-bottom:0px;border-width:1px;border-radius: 5px 5px 0 0 !important">
	<table cellspacing="0" cellpadding="0">
		<tr>
			<td class="plc pbn vwthd" style="border: 0 !important;padding-top:11px;padding-right:20px !important;padding-bottom:10px !Important">
				...
				<a href="home.php?mod=space&uid=$_G[forum_thread][authorid]" class="topavt"><!--{avatar($_G[forum_thread][authorid],middle)}--></a>
				<!--{else}-->
				<a class="topavt"><img src="$_G['style']['styleimgdir']/hidden.gif" title="$_G[setting][anonymoustext]" alt="$_G[setting][anonymoustext]" /></a>
				<!--{/if}-->
				<!--{if !IS_ROBOT}-->
				...
				<h1 class="ts" style="font-size:20px">
					...
					<!--{/if}-->
					<span id="thread_subject" style="font-weight:100">$_G[forum_thread][subject]</span>
				</h1>
				<span class="xg1">
				...
				<div class="mtn xg1" style="font-size: 12px;margin-bottom:2px;float:left">
					...
					<div style="float:left"><span>{lang forum}:</span></div>
					  <div style="float:left;overflow: hidden;width:500px"><span style="margin-left:-5px">$navigation</span>
						<span>{lang dateline}:</span><span id="authorposton$post[pid]">$post[dateline]</span>
					  </div>
				</div>
				<div class="hm ptn" style="margin-top: 0px;padding-top: 3px !important;text-align: right;">
					<span class="show xi1" title="{lang show}">$_G[forum_thread][views]</span><span class="pipe"></span><span class="reply xi1" title="{lang reply}">$_G[forum_thread][allreplies]</span>
				</div>
			</td>
			</div>
		</tr>
	...
	<!--{if $modmenu['thread']}-->
	<div id="modmenu" class="xg1 pbm bm" style="border-width:0;border-left-width:1px;border-right-width:1px;margin-bottom:0 !important;padding-left: 20px;padding-top: 10px;padding-bottom: 0px !important;border-radius:0 !Important">
		<!--{eval $modopt=0;}-->
...
<!--{/if}-->
	<div id="pgt" class="pgs mbm cl bm {if $modmenu['thread']}pbm bbs{/if}" style="margin-bottom: 0 !Important;margin-left:0px !Important;padding: 10px 20px;padding-bottom: 15px !important;border-radius:0 0 5px 5px !important;border-width:1px;border-top-width:0">
		...
		<!--{if $_G['forum']['threadsorts'] && $_G['forum']['threadsorts']['templatelist']}-->
			...
			<!--{/loop}-->
		<!--{else}-->
			<!--{if !$_G['forum_thread']['is_archived']}--><a id="newspecial" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} href="javascript:;" title="{lang send_posts}"><img src="template/enet_no1/images/pn_post.png" alt="{lang send_posts}" /></a><!--{/if}-->
		<!--{/if}-->
			<a id="post_reply" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]')" href="javascript:;" title="{lang reply}"><img src="template/enet_no1/images//pn_reply.png" alt="{lang reply}" /></a>
...
<div id="postlist" class="pl" style="margin-top:10px">
				...
				<tr>
					...
					<!--{/if}-->
						<img src="{IMGDIR}/thread_prize_s.png" class="hm" alt="{lang replycredit}" />
							{$_G['forum_thread']['replycredit']} {$_G['setting']['extcredits'][$_G[forum_thread][replycredit_rule][extcreditstype]][unit]}{$_G['setting']['extcredits'][$_G[forum_thread][replycredit_rule][extcreditstype]][title]}
					<!--{if !$close_leftinfo}-->
					</td>
	...
	<!--{loop $postlist $post}-->
		...
		<div id="pid$post[pid]" class="bm" style="border:0">
...
<!--{eval $nxtpage = $page + 1;}-->
	<div class="pgbtn" style="width:100% !important;position: relative;z-index: 0;margin-left:0"><a href="forum.php?mod=viewthread&tid=$_G[tid]{if $_GET[authorid]}&authorid=$_GET[authorid]{/if}&page=$nxtpage" hidefocus="true" class="bm_h" style="background:#f5f5f5">{lang next_page_extra}</a></div>
<!--{/if}-->
...
<div class="pgs cl" style="padding:0 20px;margin-bottom:10px">
	...
	<!--{if !$_G['forum_thread']['is_archived']}-->
		<a id="newspecialtmp" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} href="javascript:;" title="{lang send_posts}"><img src="template/enet_no1/images/pn_post.png" alt="{lang send_posts}" /></a>
	<!--{/if}-->
		<a id="post_replytmp" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]')" href="javascript:;" title="{lang reply}"><img src="template/enet_no1/images//pn_reply.png" alt="{lang reply}" /></a>
</div>
