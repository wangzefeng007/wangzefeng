...
//手机站
define('WEB_M_URL', 'http://m.longwengui.net');
//网站后台管理
...
if ($Group == 'Debt') {
    if ($_COOKIE['UserID'] != '' && $_COOKIE['Account'] != '') {
        ...
        setcookie("session_id", session_id(), time() + 3600, "/", WEB_HOST_URL);
    } else {
        //同步SESSIONID
        if ($_SESSION['UserID'] != '') {
            setcookie("session_id", session_id(), time() + 60, "/", WEB_HOST_URL);
        }
    }
}
if ($Group == 'Admin') {
