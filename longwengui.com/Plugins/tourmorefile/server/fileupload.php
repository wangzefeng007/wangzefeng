...
$InsertPicInf['ImageUrl'] = $uploadDir.'/'.$fileName;
//$InsertPicInf['TourProductID'] = $_SESSION['TourProductID'];
$InsertPicInf['TourProductID'] = trim($_GET['TourProductID']);
$DB-> insertArray('tour_product_image', $InsertPicInf);
//日志开始
if($_SERVER["HTTP_HOST"]=='admin.57us.com')
{
    if($_POST)
    {
    ...
    $MyUrl = GetMyUrl().'&ImageUrl='.$InsertPicInf['ImageUrl'];
    $MyIP = GetIP();
    $MyUserID = $_SESSION['AdminID'];
    $DateTime = date("Y-m-d H:i:s");
    file_put_contents('./../../../Log/'.date("Y-m-d").'.txt', '[!@# '.$MyUserID.' | '.$MyIP.' | '.$DateTime.' | '.$MyUrl.' | '.$GET.' | '.$POST.' #@!]
',FILE_APPEND);
}
//日志结束
